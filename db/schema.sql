...
-- Load: sqlite3 db/support.db < db/schema.sql
...
CREATE TABLE banks (
  ...
  go_live_date         DATE,
  contact_region       TEXT NOT NULL CHECK (contact_region IN ('APAC','EMEA','AMER')),
...
CREATE TABLE ticket_events (
  ...
  event_type  TEXT NOT NULL CHECK (event_type IN (
    'created','assigned','first_response','pending_customer','pending_external',
    'sla_paused','sla_resumed','escalated','solved','reopened','csat'
  )),
  actor_id    TEXT,
  note        TEXT,
...
CREATE TABLE knowledge_articles (
  ...
  domain         TEXT NOT NULL CHECK (domain IN (
    'access','status','documents','lending','sba','incident','billing','operations'
  )),
...
CREATE TABLE article_feedback (
  ...
  article_id  TEXT NOT NULL REFERENCES knowledge_articles(article_id),
...
CREATE TABLE comms_touchpoints (
  ...
  kind          TEXT NOT NULL CHECK (kind IN (
    'incident','maintenance','health_check','onboarding','csat','follow_up'
  )),
  channel       TEXT NOT NULL,
  subject       TEXT NOT NULL,
  sent_at       TEXT NOT NULL,
...
CREATE TABLE ai_deflections (
  ...
  channel       TEXT NOT NULL,
  intent        TEXT NOT NULL,
  resolver      TEXT NOT NULL CHECK (resolver IN ('help_center','chatbot','in_app','macro')),
...
CREATE INDEX idx_tickets_bank ON tickets(bank_id);
...
CREATE INDEX idx_articles_domain ON knowledge_articles(domain, audience);
CREATE INDEX idx_banks_region ON banks(contact_region);
